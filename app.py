...
import numpy as np
import telepot
bot = telepot.Bot("6420315381:AAFcnreFZaS6CdCQgnWaCOlFRdCPozCuE8k")
csid = "6021108475"
...
def crop_recommend():
    import requests
    import pandas as pd
    data=requests.get("https://api.thingspeak.com/channels/2513579/feeds.json?api_key=X9EDTD2E15N0AQI1&results=2")
    ...
    return render_template('crop.html', title=title,n=n,p=p,k=k,temp=temp,hum=hum,moi=moi)
...
def yeild():
    ...
    # , temp=temp, hum=hum)
    return render_template('crop_yeild.html', title=title,n=n,p=p,k=k,temp=temp,hum=hum,moi=moi)
...
def yeild_predict():
    ...
    if request.method == 'POST':
        ...
        out_1 = forest.predict([[float(state),
                                 ...
                                 float(soilmoisture),
                                 float(area)]])
        if float(soilmoisture) < 200:
            bot.sendMessage(csid,str(f" soil moisture {soilmoisture}  ie.   \n Water content is low "))
...
def crop_fer():
    ...
    return render_template('fer.html', title=title,n=n,p=p,k=k,temp=temp,hum=hum,moi=moi ) #,n=n)
